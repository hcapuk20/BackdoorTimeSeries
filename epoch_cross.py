...
def create_cross(inputs1, inputs2, bd_model):
    patterns2, patterns2_clipped = bd_model(inputs2)
    inputs_cross = inputs1 + (patterns2_clipped - inputs1)
    return inputs_cross, patterns2_clipped


def epoch_marksman_lam_cross(bd_model, bd_model_prev, surr_model, loader1, args, loader2=None, opt_trig=None, opt_class=None,train=True, mp_scaler=None):
    total_loss = []
    all_preds = []
    bd_preds = []
    trues = []
    ...
    for i, (batch_x, label, padding_mask), (batch_x2, label2, padding_mask2) in zip(range(len(loader1)), loader1,
        ...
        with autocast_if_needed(device_type="cuda", enabled=mp_scaler is not None):
            trigger, trigger_clip = bd_model_prev(batch_x, padding_mask, None, None,
                                                  bd_labels)  # generate trigger with frozen model
            mask = (label != bd_label).float().to(args.device) if args.attack_only_nontarget else torch.ones_like(
                label).float().to(args.device)
            mask = mask.unsqueeze(-1).expand(-1, trigger_clip.shape[-2], trigger_clip.shape[-1])
            batch_mix, scale_weights = mixup_class(batch_x, batch_x + trigger_clip * mask, args.lambda_alpha,
                                                   args.lambda_beta)  # generate mix_batch
        ...
        with autocast_if_needed(device_type="cuda", enabled=mp_scaler is not None):
            bd_pred = surr_model(batch_x + trigger_clip * mask, padding_mask, None,
                                 None)  # surrogate classifier in eval mode
            ######## here we combine two loss one for each label
            loss_bd = nn.CrossEntropyLoss(label_smoothing=args.label_smooth)(bd_pred, bd_labels.long().squeeze(
                -1))  # output size of batch
            # loss_clean = args.criterion_mix(bd_pred, label.long().squeeze(-1)) # output size of batch
